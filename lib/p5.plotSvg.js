    ...
    _p5Instance.text = function(content, x, y, maxWidth, maxHeight) {
      if (_bRecordingSvg) {
        ...
        if (typeof currentFont === 'string') {
          ...
          let fontName = currentFont.font.names.fontFamily ||
            currentFont.font.names.fullName ||
            currentFont.font.names.postScriptName;

          // Font names are someetimes objects with language codes, extract the actual string
          if (fontName && typeof fontName === 'object') {
            font = fontName.en || fontName[Object.keys(fontName)[0]];
          } else {
            font = fontName;
          }
        }
